/** biome-ignore-all lint/suspicious/noExplicitAny: Types mismatch bw solidjs and html */
...
type Color = Record<Varaint, { background: string; text: string; border: string }>;

const moveToastsUp = (gap?: number) => {
  const toasts: NodeListOf<HTMLDivElement> = document.querySelectorAll(".toast");
  ...
  for (const toast of toasts) {
    if (toast.classList.contains("newest")) {
      ...
      const newValue = Number.parseInt(prevValue, 10) + toastHeight + (gap ?? 16);
...
const moveToastsDown = (closingToast: HTMLElement, gap?: number) => {
  ...
  const closingToastBottom = Number.parseInt(closingToast.style.bottom.replace("px", ""), 10);

  for (const toast of toasts) {
    if (toast === closingToast) continue;

    const toastBottom = Number.parseInt(toast.style.bottom.replace("px", ""), 10);
...
const animateToastExit = (popover: HTMLElement, options?: ToastOptions) => {
  ...
  const transitionDuration = options?.transitionDuration ? options.transitionDuration : 250;
  ...
  createRoot((disposer) => {
    ...
    const popover = (
      <article
        ...
        style={{
          "--transition-duration": options?.transitionDuration ? `${options?.transitionDuration}ms` : "250ms",
