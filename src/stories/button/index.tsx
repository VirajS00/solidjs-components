...
type Props<T extends ValidComponent, P = ComponentProps<T>> = {
  ...
  color?: "default" | "secondary" | "monochrome" | "warning" | "success" | "error" | "ghost";
  variant?: "filled" | "outline" | "link";
  as?: T;
  class?: string;
};

export const Button = <T extends ValidComponent = "button">(props: Props<T>) => {
  // biome-ignore lint/style/noParameterAssign: Merge Props
  props = mergeProps({ color: "default", variant: "filled", as: "button" }, props);

  const [local, rest] = splitProps(props, ["class", "color", "variant", "as"]) as [
    Pick<Props<T>, "color" | "class" | "as" | "variant">,
    Props<T>,
  ];
