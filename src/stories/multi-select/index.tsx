...
export const MultiSelect: Component<Props> = (props) => {
  const [local, rest] = splitProps(props, ["value", "options", "placeholder", "class", "classList", "style", "ref"]);
  ...
  onMount(() => {
    if (local.value && local.value.length > 0) {
      ...
      setSelected(local.options.filter((x) => inputRef?.defaultValue.split(",").includes(x.value)));
    }
  ...
  return (
    <>
      <Listbox multiple toggleable defaultOpen={false} value={selected()} onSelectChange={setSelected}>
        <div class={styles.containerDiv}>
          ...
          >
            <div class={styles.tagsContainer}>
              <For
                each={selected()}
                fallback={<span class={styles.placeholder}>{local.placeholder ?? "Select Values"}</span>}
              >
                {(item): JSX.Element => (
                  <span class={styles.tag}>
                    <span>{item.label}</span>
                    {/** biome-ignore lint/a11y/noStaticElementInteractions: mouseEvent prevention */}
                    {/** biome-ignore lint/a11y/useAriaPropsSupportedByRole: Role inside component */}
                    <span
                      // biome-ignore lint/a11y/noNoninteractiveTabindex: Need for keyboard navigation
                      tabIndex={0}
