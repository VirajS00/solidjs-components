...
const ExampleStory = () => {
  // biome-ignore lint/suspicious/noExplicitAny: Form type has any
  ...
  return (
    <Form class={styles.formContainer} data={DummyFormData} onSubmit={handleSubmit}>
      <h2>Sample Form</h2>
      ...
      <div class={styles.inputContainer}>
        ...
        <Select options={formOptions.countrySelection} id="countrySelection" name="countrySelection" />
      </div>
      <div class={styles.inputContainer}>
      ...
      <div class={styles.inputContainer}>
        ...
        <MultiValueInput id="favoriteWebsites" name="favoriteWebsites" placeholder="Enter URL" />
      </div>
      <div class={styles.inputContainer}>
          ...
          <For each={formOptions.preferredContactMethod}>
            {(item, i) => (
              <div style={{ display: "flex", "align-items": "center" }}>
                <RadioButton id={`preferredContactMethod-${i()}`} name="preferredContactMethod" value={item.value} />
                <Label for={`preferredContactMethod-${i()}`} style={{ "padding-left": "0.5em" }}>
                  {item.label}
                </Label>
              </div>
            )}
      ...
      <div class={styles.inputContainer}>
        ...
        <div class={styles.inputContainer}>
          <For each={formOptions.notificationSettings}>
            {(item, i) => (
              <div style={{ display: "flex", "align-items": "center" }}>
                <Checkbox id={`notificationSettings-${i()}`} name="notificationSettings" value={item.value} />
                <Label for={`notificationSettings-${i()}`} style={{ "padding-left": "0.5em" }}>
                  {item.label}
                </Label>
              </div>
            )}
      ...
      <div class={styles.inputContainer}>
        ...
        <div class={styles.inputContainer}>
          <For each={formOptions.favoriteAnimal}>
            {(item, i) => (
              <div style={{ display: "flex", "align-items": "center" }}>
                <Checkbox id={`favoriteAnimal-${i()}`} name="favoriteAnimal" value={item.value} />
                <Label for={`favoriteAnimal-${i()}`} style={{ "padding-left": "0.5em" }}>
                  {item.label}
                </Label>
              </div>
            )}
