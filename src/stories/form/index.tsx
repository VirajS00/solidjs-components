/** biome-ignore-all lint/suspicious/noExplicitAny: Form Data can be any */
import { type Component, type JSX, onMount, splitProps } from "solid-js";

const setInitialValues = (form: HTMLFormElement, data: any) => {
  const formElements: NodeListOf<HTMLInputElement> | undefined = form.querySelectorAll("[name]");
...
const getFormValues = (form: HTMLFormElement) => {
  const vals = Object.fromEntries(new FormData(form)) as Record<string, any>;

  const arrayElements: NodeListOf<HTMLInputElement> = form.querySelectorAll("[data-type='array']");

  for (const el of arrayElements) {
    vals[el.name] = el.value.split(",");
  }

  const dualRangeElements: NodeListOf<HTMLInputElement> = form.querySelectorAll("[data-type='range-array']");
  ...
  const numberVals: NodeListOf<HTMLInputElement> = form.querySelectorAll("[data-type='number']");

  for (const el of numberVals) {
    vals[el.name] = Number.parseFloat(el.value);
  }

  const dateVals: NodeListOf<HTMLInputElement> = form.querySelectorAll("[data-type='date']");
  ...
  const switchesVals: NodeListOf<HTMLInputElement> = form.querySelectorAll("input[type='checkbox'][role='switch']");
