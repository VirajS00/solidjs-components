import { type Component, createEffect, createSignal, For, type JSX, Show, splitProps } from "solid-js";
...
export const Combobox: Component<Props> = (props) => {
  const [local, rest] = splitProps(props, ["id", "options", "ref", "onInput", "label"]);
  ...
  createEffect(() => {
    ...
    const currentEl: HTMLLIElement | null | undefined = pannelRef?.querySelector(`#lb-option-${random}-${item.index}`);
    ...
    <Popover as="div" defaultOpen={false} class={styles.comboWrap}>
      {({ isOpen, close, open }): JSX.Element => {
        ...
        document.addEventListener("click", (e) => {
          ...
          const isOutsidePanel = target !== pannelRef && !pannelRef?.contains(target);
          ...
          <>
            <div class={styles.inputWrap}>
              <input
                ...
                aria-activedescendant={currentItem().index === -1 ? "" : `lb-option-${random}-${currentItem().index}`}
                onInput={(e) => {
                  ...
                  if (inputValue !== "") {
                    const els = pannelRef?.querySelectorAll<HTMLLIElement>("li");
                    ...
                    if (els) {
                      ...
                      if (els[0]) {
                        ...
                        e.target?.setAttribute("aria-activedescendant", els[0]?.id ?? "");
                      }
                    }
                  }

                  const newOpt = local.options.filter((o) => o.toLowerCase().includes(inputValue.toLowerCase()));
                ...
                onKeyPress={(e) => {
                  if (e.key === "Enter") {
                    e.preventDefault();
                    const currentFocused = pannelRef?.querySelector<HTMLLIElement>("li[data-focus='true']");
            ...
            >
              ...
              >
                <For
                  each={opts()}
                  fallback={<span class={`${styles.placeholder} ${styles.addPadding}`}>No results found.</span>}
                >
                  {(option, i) => (
                    // biome-ignore lint/a11y/useFocusableInteractive: Listbox element
                    // biome-ignore lint/a11y/useKeyWithClickEvents: Listbox element
                    <li
                      class={`${styles.option} lb-option`}
                      // biome-ignore lint/a11y/noNoninteractiveElementToInteractiveRole: Listbox element
                      role="option"
