...
const findItem = (items: NodeListOf<HTMLButtonElement>, target: HTMLButtonElement) => {
...
const findClosestDay = (buttons: NodeListOf<HTMLButtonElement>, day: number): HTMLButtonElement | null =>
  Array.from(buttons).find((button) => Number(button.getAttribute("data-day")) === day) ||
  Array.from(buttons).reduce((closest, button) => {
    const buttonDay = Number(button.getAttribute("data-day"));
    return Math.abs(buttonDay - day) < Math.abs(Number(closest.getAttribute("data-day"))) ? button : closest;
  }, buttons[0]);

// TODO: Add multiple date select

export const Calendar: Component<Props> = (props) => {
  // biome-ignore lint/style/noParameterAssign: solidjs merge props
  ...
  const monthDays = createMemo(() => {
    ...
    for (let day = 0; day < 42; day += 1) {
      ...
      const calendarDay = {
        ...
        selected: firstDay.toDateString() === props.selectedDay().toDateString(),
  ...
  onMount(() => {
    const buttons: NodeListOf<HTMLButtonElement> | undefined = buttonsContainerRef?.querySelectorAll(".calendar-day");
    ...
    buttonsContainerRef?.addEventListener("keydown", (e) => {
      ...
      if (increment !== 0) {
        const active: HTMLButtonElement | null = buttonsContainerRef?.querySelector(
        ...
        if (newIndex >= 0 && newIndex < buttons.length) {
          ...
          if (buttons[newIndex].getAttribute("data-prev-month") === "true") {
            setCurrentDay(new Date(currentDay().getFullYear(), currentDay().getMonth() - 1, currentDay().getDate()));
  ...
  return (
    ...
    >
      <div class={styles.buttonsContainer}>
        <button
          type="button"
          disabled={!!props.startYear && year() <= props.startYear && month() === 0}
          onClick={() => {
            setCurrentDay(new Date(currentDay().getFullYear(), currentDay().getMonth() - 1, currentDay().getDate()));
          }}
        ...
        <button
          type="button"
          disabled={!!props.endYear && year() >= props.endYear && month() === 11}
          onClick={() => {
            setCurrentDay(new Date(currentDay().getFullYear(), currentDay().getMonth() + 1, currentDay().getDate()));
          }}
      ...
      <div class={styles.monthGrid} style={{ position: "relative" }} ref={buttonsContainerRef}>
        <Index each={monthDays()}>
          {(day) => (
            // biome-ignore lint/a11y/useAriaPropsSupportedByRole: Selectable element should be button
            <button
              type="button"
              onClick={() => {
              ...
              classList={{
                ...
                [styles.selectedOtherMonth]: day().selected && !day().currentMonth,
