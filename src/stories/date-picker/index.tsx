...
export const DatePicker: Component<Props> = (props) => {
  ...
  // biome-ignore lint/style/noParameterAssign: mergeProps
  props = mergeProps({ selectLayout: true }, props);
  const [local, rest] = splitProps(props, ["ref", "startYear", "endYear", "selectLayout"]);
  ...
  onMount(() => {
    ...
    if (form) {
      form.addEventListener("reset", () => {
        ...
        dateInputRef.defaultValue = new Date(Number.parseFloat(inputRef.defaultValue) * 1000)
          .toISOString()
          .substring(0, 10);
      });
    }
  ...
  const currentDate = createMemo(() =>
    new Date(selectedDay().getTime() - selectedDay().getTimezoneOffset() * 60000).toISOString().substring(0, 10),
  );
          ...
          <>
            <div class={styles.inputContainer}>
              <input
                ...
                ref={dateInputRef}
              />
              <PopoverButton type="button" aria-label="Select Date" class={styles.pickerButton} ref={buttonRef}>
            ...
            >
              <PopoverPanel unmount={false} class={styles.innerPannel} ref={pannelRef}>
                <Calendar
