...
export const Select: Component<Props> = (props) => {
  ...
  const selectedVal = createMemo(() => local.options.find((x) => x.value === selected()));
  ...
  return (
    <>
      <Listbox defaultOpen={false} multiple={false} value={selected() ?? ""} onSelectChange={handleSelectChange}>
        <div class={`${styles.containerDiv} custom-select`}>
          ...
          >
            <Show
              when={selectedVal()}
              fallback={<span class={styles.placeholder}>{local.placeholder ?? "Select Option"}</span>}
            >
            ...
            {({ isOpen }): JSX.Element => (
              ...
              >
                <ListboxOptions
                  ...
                  classList={{
                    [local.optionsContainerClass ?? ""]: !!local.optionsContainerClass,
                  }}
                >
